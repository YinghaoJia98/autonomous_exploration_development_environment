...
void scanHandler(const sensor_msgs::PointCloud2::ConstPtr& scanIn)
{
  ...
  // Transform the point cloud from the sensor frame to the world frame
  sensor_msgs::PointCloud2 cloud_out;
  try {
      // Wait for the transform to be available
      tf_listener.waitForTransform("world", scanIn->header.frame_id, 
                                     scanIn->header.stamp, ros::Duration(1.0));

      // Transform the point cloud
      pcl_ros::transformPointCloud("world", *scanIn, cloud_out, tf_listener);

      // cloud_out now contains the point cloud in the 'world' frame
      //ROS_INFO("Successfully transformed point cloud to 'world' frame.");
        
      // Process cloud_out as needed
  ...
  scanData2.header.stamp = ros::Time().fromSec(scanTime);
  scanData2.header.frame_id = "world";
  pubScanPointer->publish(cloud_out);
}
